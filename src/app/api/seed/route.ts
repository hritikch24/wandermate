...
// GET /api/seed — seed database with sample data (call once after deploy)
export async function GET() {
  try {
    ...
    await Promise.all([
      ...
      prisma.trip.create({ data: { userId: users[4].id, destination: 'kerala', title: 'Luxury Kerala Backwaters', description: "I'll sponsor the entire trip - houseboat, Ayurveda spa, tea plantation tour.", startDate: new Date('2026-09-01'), endDate: new Date('2026-09-08'), sponsorType: 'sponsor', budget: '₹50,000', interests: JSON.stringify(['Fine Dining', 'Luxury Stays', 'Photography']) } }),
      prisma.trip.create({ data: { userId: users[5].id, destination: 'goa', title: 'Beach Vibes on a Budget', description: 'Looking for someone to split hostel costs and explore hidden beaches.', startDate: new Date('2026-07-10'), endDate: new Date('2026-07-17'), sponsorType: 'need-sponsor', budget: '₹4,000', interests: JSON.stringify(['Music', 'Beaches', 'Street Food']) } }),
      prisma.trip.create({ data: { userId: users[6].id, destination: 'nepal', title: 'Annapurna Photography Expedition', description: 'Serious photography trip to capture the Himalayas.', startDate: new Date('2026-10-01'), endDate: new Date('2026-10-15'), sponsorType: 'just-join', budget: '₹35,000', interests: JSON.stringify(['Wildlife', 'Photography', 'Camping']) } }),
      prisma.trip.create({ data: { userId: users[7].id, destination: 'nepal', title: 'Kathmandu Temple Trail', description: 'Exploring the ancient temples and architecture of the Kathmandu Valley.', startDate: new Date('2026-11-05'), endDate: new Date('2026-11-15'), sponsorType: 'sponsor', budget: '₹25,000', interests: JSON.stringify(['History', 'Architecture', 'Museums']) } }),
    ]);

    // Create guide users + profiles
    const guideUsers = await Promise.all([
      prisma.user.create({ data: { name: 'Deepak Rawat', email: 'deepak.rawat@example.com', passwordHash, age: 34, bio: 'Born and raised in Mussoorie. I know every hidden trail, secret waterfall, and the best chai stalls.', languages: JSON.stringify(['Hindi', 'English', 'Garhwali']), verified: true, rating: 4.9 } }),
      prisma.user.create({ data: { name: 'Lakshmi Menon', email: 'lakshmi.menon@example.com', passwordHash, age: 29, bio: "Ayurveda practitioner and backwater expert. I'll show you the Kerala that guidebooks miss.", languages: JSON.stringify(['Malayalam', 'Hindi', 'English', 'Tamil']), verified: true, rating: 5.0 } }),
      prisma.user.create({ data: { name: 'Tenzing Sherpa', email: 'tenzing@example.com', passwordHash, age: 38, bio: 'Third-generation mountain guide. Summited Annapurna twice.', languages: JSON.stringify(['Nepali', 'Hindi', 'English', 'Tibetan']), verified: true, rating: 4.9 } }),
      prisma.user.create({ data: { name: 'Ravi Kumar', email: 'ravi.kumar@example.com', passwordHash, age: 31, bio: 'Rafting instructor and certified adventure guide. Grade 4 rapids to cliff jumps.', languages: JSON.stringify(['Hindi', 'English']), verified: true, rating: 4.8 } }),
      prisma.user.create({ data: { name: 'Fernandes Gomes', email: 'fernandes@example.com', passwordHash, age: 36, bio: 'Goan born, Portuguese roots. Secret forts, spice plantations, best fish curry.', languages: JSON.stringify(['Konkani', 'Hindi', 'English', 'Portuguese']), verified: true, rating: 4.7 } }),
      prisma.user.create({ data: { name: 'Pema Dorji', email: 'pema@example.com', passwordHash, age: 27, bio: 'Former monk turned guide. Spiritual lens to every Himalayan trek.', languages: JSON.stringify(['Hindi', 'English', 'Tibetan']), verified: true, rating: 4.8 } }),
      prisma.user.create({ data: { name: 'Himanshu Thakur', email: 'himanshu@example.com', passwordHash, age: 33, bio: 'Snow sports instructor and Manali local. First time skiing or secret powder spots.', languages: JSON.stringify(['Hindi', 'English', 'Pahari']), verified: true, rating: 4.9 } }),
      prisma.user.create({ data: { name: 'Mohan Negi', email: 'mohan@example.com', passwordHash, age: 40, bio: 'Retired army man. Expert in high-altitude treks and survival skills.', languages: JSON.stringify(['Hindi', 'English', 'Pahari']), verified: true, rating: 5.0 } }),
    ]);
    ...
    return NextResponse.json({
      ...
      created: { users: 16, trips: 8, guides: 8 },
      demoLogin: { email: 'arjun@example.com', password: 'password123' },
    });
  } catch (error) {
